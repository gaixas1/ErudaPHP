...
class Eruda_Controller_Admin extends Eruda_Controller {
    ...
    public function ini() {
        ...
        if(!$this->_onlyheader) {
            ...
            $this->header->setBaseURL(Eruda_Environment::getBaseURL().'admin/');
            $this->header->setMetatag('Description', 'FallenSoulFansub, todos nuestros mangas on-line para tu disfrute.');
            $this->header->append2Title(Eruda_Environment::getTitle().' Admin');
    ...
    function Index() {        
        if(!$this->_onlyheader) {
            $dir = array(
                '' => Eruda_Environment::getTitle(). ' Administración'
                );
