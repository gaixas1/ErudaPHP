...
<p>Se ha generado una solicitud de Recuperación de Contraseña desde nuestro Sitio Web - '.Eruda_Environment::getTitle().' <br/>
Si usted no ha solicitado este mensaje, por favor ignórelo. Si sigue recibiéndolo contáctenos 
a <strong>'.Eruda_Environment::getMail().'</strong>.</p>
<p>Sigue este enlace para recuperar el control de tu cuenta: <br/>
<a href="'.Eruda_Environment::getBaseURL().'user/recupera/'.$user->get_id().'-'.$rand.'/">'.Eruda_Environment::getBaseURL().'user/recupera/'.$user->get_id().'-'.$rand.'/</a> <br/>
...
<p><strong>Gracias por seguirnos<strong></p>
    ...
    function RecuperaError() {      
        ...
        if(!$this->_onlyheader) {
            ...
            $model->set_ref(Eruda_Environment::getBaseURL());
    ...
    function setRefered(){
        if(isset($_SERVER['HTTP_REFERER']) && strlen($_SERVER['HTTP_REFERER'])>0 && !preg_match('~/user/.*$~', $_SERVER['HTTP_REFERER'])){
            ...
            $this->refered = Eruda_Environment::getBaseURL();
            $_SESSION['REFERER'] = $this->refered;
        }
    }
