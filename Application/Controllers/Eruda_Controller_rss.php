...
class Eruda_Controller_RSS {
    ...
    function index() {        
        if(!$this->_onlyheader) {
            $entries = Eruda_Mapper_Entry::getFromAll(0, Eruda::getEnvironment()->getEntriesPerPage()*5);
            self::setEntries($entries);
            
            $model = new Eruda_Model_RSS(Eruda::getEnvironment()->getTitle(), Eruda::getEnvironment()->getBaseURL(), 'es-es', 'FallenSoul Fansub', 'ErudaBlog', $entries);
