...
class Eruda_Controller_Entrada extends Eruda_Controller{
    ...
    public function ini() {
        ...
        if(!$this->_onlyheader) {
            ...
            $this->header->addRSS(Eruda_Environment::getBaseURL().'rss.xml');
        }
    }
    ...
    function index() {        
        if(!$this->_onlyheader) {
            $entries = Eruda_Mapper_Entry::getFromAll(0, Eruda_Environment::getEntriesPerPage());
            self::setEntries($entries);
            
            $page = new Eruda_Model_Page('/', 1, ceil(Eruda_Mapper_Entry::countFromAll()/Eruda_Environment::getEntriesPerPage()));
    ...
    function Paginacion() {        
        if(!$this->_onlyheader) {
            ...
            $maxPages = ceil(Eruda_Mapper_Entry::countFromAll()/Eruda_Environment::getEntriesPerPage());
            ...
            $entries = Eruda_Mapper_Entry::getFromAll(($pag-1)*Eruda_Environment::getEntriesPerPage(), Eruda_Environment::getEntriesPerPage());
    ...
    function CategoriaIndex() {        
        if(!$this->_onlyheader) {
            ...
            $maxPages = ceil($categoria->get_count()/Eruda_Environment::getEntriesPerPage());
            
            $entries = Eruda_Mapper_Entry::getFromCat($categoria->get_id(), 0, Eruda_Environment::getEntriesPerPage());
            self::setEntries($entries);
    ...
    function CategoriaPaginacion() {        
        if(!$this->_onlyheader) {
            ...
            $entries = Eruda_Mapper_Entry::getFromCat($categoria->get_id(), ($pag-1)*Eruda_Environment::getEntriesPerPage(), $maxPages);
            
            self::setEntries($entries);
            
            $page = new Eruda_Model_Page('/'.$catLink.'/', $pag, ceil($categoria->get_count()/Eruda_Environment::getEntriesPerPage()));
