...
class Eruda_Model_Manga extends Eruda_Model {
    ...
    public function __toString() {
        ...
        if($this->verO)
            $a .=  '<img src="'.Eruda_Environment::getBaseURL().self::$URLc.$this->id.'a.jpg"/> <img src="'.Eruda_Environment::getBaseURL().self::$URLc.$this->id.'b.jpg"/>';
        else
            $a .=  '<img src="'.Eruda_Environment::getBaseURL().self::$URLc.$this->id.'a.jpg"/>';
        $a .= '</section>';
            
        $a .= '<footer>';
        
        if($this->verO)
            $a .=  '<a title="Ver online" target="_blank" href="'.self::$VERONLINE.'ver.php?pg='.$this->serie.'/'.$this->tomo.'/'.$this->titulo.'"><img class ="donwImage"  alt="Ver online" src="'.Eruda_Environment::getBaseURL().self::$URLt.'ver.jpg"></a>';
            
        foreach($this->links as $k => $link) {
            if (isset(self::$servers[$link[0]])) 
                if(self::$servers[$link[0]]==1)
                    $a .=  '<a title="Descargar '.$link[0].'" target="_blank" href="'.self::$URLd.'manga/'.$this->id.'/'.$k.'/'.$this->serie."/".$this->tomo.'/"><img class ="donwImage"  alt="Descargar '.$link[0].'" src="'.Eruda_Environment::getBaseURL().self::$URLt.$link[0].'.jpg"></a>';
                else if(self::$servers[$link[0]]==2)
                    $a .=  '<a title="Descargar Torrent" target="_blank" href="'.self::$URLd.'manga/'.$this->id.'/'.$k.'/'.$this->serie."/".$this->tomo.'/"><img class ="donwImage"  alt="Descargar Torrent" src="'.Eruda_Environment::getBaseURL().self::$URLt.'torrent.jpg"></a>';
                else
                    $a .=  '<img class ="donwImageOff"  alt="Descargar '.$link[0].'" src="'.Eruda_Environment::getBaseURL().self::$URLt.$link[0].'.jpg">';
            else
                $a .=  '<a title="Descargar '.$link[0].'" target="_blank" href="'.self::$URLd.'manga/'.$this->id.'/'.$k.'/'.$this->serie."/".$this->tomo.'/"><img class ="donwImage" alt="Descargar '.$link[0].'" src="'.Eruda_Environment::getBaseURL().self::$URLt.'default.jpg"></a>';
        }
